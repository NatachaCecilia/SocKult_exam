...
#plot(hypothesis(m_emp, "conditioncontrol > 0", class = "sd"))
#Look at the sd, see riccardos code
...
xx <- predict(m_emp, summary=T)
d_hej <- cbind(d,xx)
d_hej$ID <- as.factor(d_hej$ID)
ggplot(d_hej) + 
  geom_point(aes(condition, empathy, color = ID, group=ID)) + 
  geom_boxplot(aes(condition, empathy, color = ID))
  

X <- hypothesis(m_emp, "conditionimagine_other > conditionimagine_self", group = "ID", scope = "coef")
X$hypothesis %>%
  left_join(distinct(d_hej, Group = ID)) %>% 
