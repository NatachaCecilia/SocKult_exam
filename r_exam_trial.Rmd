...
#Load libraries
library(pacman)
p_load(tidyverse, tidyr, reshape2, data.table, dplyr, Hmisc, viridis)
...
#Finding the number of participants, gender distribution, mean age, sd of mean age, min and max age
d_n$ID <- factor(d_n$ID)
unique(d_n$ID)

information <- d_n %>%
 group_by(gender) %>%
 summarise(gender_count=n()/3, mean_age = mean(age), sd_age = sd(age))
...
# Define the prior
prior_mind <- c(
prior(normal(0,1), class = Intercept), #Intercept is still AltercentricIntrusion
prior(normal(0,.3), class = b), #Beta is Mind reading
prior(normal(1,2), class = sigma))

# Draw the consequences of priors AND likelihood
m_mind_prior_only <- brm(m_mind,
patients,
prior = prior_mind,
sample_prior = "only",
iter = 2000,
...
pp_check(m_mind_prior_only, nsamples = 100)
...
mcmc_trace(m1,  pars = "b_condition1", "b_condition2") + theme_classic()
mcmc_rank_overlay(m1, pars = "b_Intercept", "b_condition1") + theme_classic()

# Has the model learned? The posteriors have moved or gotten more confident than the priors
plot(hypothesis(m5,"condition1> 0"))
plot(hypothesis(m5,"condition2 > 0"))
plot(hypothesis(m5,"condition3> 0"))

# The posteriors have moved or gotten more confident than the priors
plot(hypothesis(m1,"condition1 > 0"))

# Draw the consequences of priors AND likelihood
m_mind_prior <- brm(m_mind,
patients,
prior = prior_mind,
sample_prior = T,
ilter = 2000,
...
pp_check(m_mind_prior, nsamples = 100)

#cheking the model
m_mind_prior

# Hypothesis testing + updating check
plot(hypothesis(m_mind_prior,
           "MindReading > 0"))

hypothesis(m_mind_prior,
           "MindReading > 0")

#Plots
conditional_effects(m_mind_prior)
plot(conditional_effects(m_mind_prior), points=T)
...
d <- filter(d, condition == "imagine_other")
d <- filter(d, condition == "imagine_self")
